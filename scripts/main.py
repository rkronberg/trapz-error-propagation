...
def main():
    ...
    try:
        ...
        print('Check input dims. Shape (n, 3) containing xdata, \
            ydata and yerror required.')
        quit()
    except OSError:
    ...
    np.savetxt('integral.out', np.c_[xdata, tz.integral, tz.error],
               header='x, F(x), error')

    # Visualize results
    if(isplot):
        plt.plot(xdata, tz.integral, '.-')
        plt.fill_between(xdata, tz.integral-tz.error, tz.integral+tz.error,
                         alpha=0.25)
        plt.show()
